...
\begin{outline}
  本章介绍了Linux操作系统的基本概念、获取方式、基本操作和文件系统等内容，并带领大家安装Arch Linux和WSL。通过学习本章内容，读者将能够了解Linux的优势和特点，掌握基本的Linux命令和文件管理方法，为后续的编程和开发工作打下基础。
\end{outline}
...
\texttt{apt} 是Debian及其衍生发行版（如Ubuntu）中用于管理软件包的工具。它可以用来安装、更新和删除软件包。如果用的是其他发行版，则需要使用相应的包管理器，例如Red Hat系使用 \texttt{yum} 或 \texttt{dnf}，Arch系使用 \texttt{pacman}。以Arch为例：
\begin{lstlisting}
sudo pacman -S sl && sl
\end{lstlisting}

这里的 \texttt{sudo} 是“以超级用户身份运行命令”的意思。因为安装软件包需要修改系统文件，所以需要管理员权限。执行这个命令时，系统会提示输入当前用户的密码，以验证权限。

让我们看看上面内容是怎么发生的。我们刚刚输入的单条命令大概是这样的形式：
\begin{lstlisting}[language=bash]
程序 [子命令] [选项] [对象]
\end{lstlisting}

其中，除了程序是必须的，其他的子命令、选项和对象都是可选的。我们用 \texttt{apt install -y sl} 来举例说说：apt是程序，install是子命令，-y是选项，sl是对象。

而后面这个 \texttt{\&\&} 的意思是“如果前面的命令成功执行了，那么就执行后面的命令”。因此，只有当 \texttt{apt install -y sl} 成功安装了 \texttt{sl} 软件包之后，才会执行 \texttt{sl} 命令来显示小火车动画。

于是我们就能看到，一个程序就这么跑起来了，看起来很明确。
...
\subsection{再临终端命令行}\label{sec:shell}
...
\subsection{实用例：用WSL配置C/C++环境}\label{sec:wsl-cpp}

我们已经介绍过了如何在Windows上配置C/C++环境，这一过程虽说不上繁琐，但足以让新手望而却步。使用WSL，可以大大简化这个过程。
