...
class Person:
    ...
    def greet(self) -> None:
        print(f"Hello, my name is {self.name} and I am {self.age} years old.")
\end{lstlisting}
上述例子又被称为“数据类”（dataclass），在Python 3.7中引入。使用数据类可以更方便地定义类，从而减少重复且意义有限的代码。“装饰器”是Python的一种语法糖，可以在函数或类定义前使用@符号来应用装饰器。数据类装饰器会自动为类生成一些常用方法，如\texttt{\_\_init\_\_()}、\texttt{\_\_repr\_\_()}等。
