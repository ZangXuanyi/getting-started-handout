...
\begin{outline}
  本章介绍了Python编程语言的基本语法和常用特性，涵盖了变量、运算、输入输出、控制流程、复合数据类型、函数和模块、文件操作以及面向对象编程等内容。通过学习本章内容，读者将能够快速上手Python编程，掌握基本的编程技巧，为后续的编程学习打下坚实的基础。
\end{outline}

\begin{prerequisite}
  \begin{itemize}
    \item 计算机的基本操作技能
    \item 基本的编程概念（变量、数据类型、控制流程等）
    \item 在任意操作系统上配置Python环境的能力
    \item 使用终端的能力
    \item 了解C/C++的基本语法（可选，但推荐）
  \end{itemize}
\end{prerequisite}
