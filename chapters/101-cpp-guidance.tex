...
\begin{problemset}
    \item 编写一个程序，将摄氏度和华氏度互转。摄氏度C和华氏度F的转换公式为：$F = C \times \frac{9}{5} + 32$，$C = (F - 32) \times \frac{5}{9}$。输入格式为：一个字符F或者C，表示输入是华氏度还是摄氏度；接下来一行是一个整数，表示温度的数值。输出格式为：一个字符F或者C，表示输出是华氏度还是摄氏度；接下来一行是转换后温度的数值，保留两位小数。提示：使用cout处理小数点后两位，可以包含头文件\texttt{<iomanip>}使用\texttt{std::fixed}和\texttt{std::setprecision(2)}，将这两个东西直接插在cout后面就行了。（难度：简单）
    \item 编写一个程序，计算一个班级期末成绩的平均分。期末成绩由平时成绩和考试成绩共同决定，其中前者占30\%，后者占70\%。输入格式为：第一行是学生人数n，接下来n行，每行输入两个整数，分别表示平时成绩和考试成绩。输出格式：一行，输出学生的平均分，保留两位小数。（难度：中等）
    \item 有一批药剂需要浓缩。每瓶药剂都有自己的浓度，每一次浓缩需要将两瓶药剂放在一起，成本为药剂浓度之和。为了保证成本最小，我们使用贪心算法，也就是每次取出浓度最低的两瓶药剂进行浓缩\footnote{这个算法的正确性可以利用排序不等式证明，这里我就不证明了}。输入格式为：第一行是药剂的数量n，接下来n行，每行一个整数，表示药剂的浓度。输出格式：一行，输出最小的总成本。（难度：困难）
\end{problemset}
