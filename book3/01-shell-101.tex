...
需要注意的是，终端大多不支持鼠标点击，因此需要全程使用键盘输入命令，用方向键的左右键移动光标（命令较长时，使用Ctrl+左右键进行以单词为单位的移动）。方向键的上下键可以用来浏览历史输入过的命令，这对于重复输入相似命令非常有用。有时候，终端会帮我们提前补全一些命令（表现为半透明文字），我们只需要按下Tab键即可自动补全命令或者文件名。

有的命令执行时间很长，或执行时遇到了错误需要掐断。这时，我们可以按下 \texttt{Ctrl+C} 来终止当前命令的执行。而如果我们仅仅是希望暂停而非终止当前命令的执行，可以按下 \texttt{Ctrl+Z} ，这会将当前命令暂停并挂起，然后你可以输入其他命令。

要恢复暂停的命令，可以使用 \texttt{fg} 命令将其恢复到前台继续执行。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{shell-long-cmd.png}
    \caption{在Linux上运行一个执行时间较长的命令。使用 \texttt{Ctrl+C} 来中断它的执行；使用 \texttt{Ctrl+Z} 来暂停它的执行；使用 \texttt{fg} 来恢复它的执行。}
\end{figure}

有的时候，命令执行时间实在是太长了，我们不想等着这东西一直运行下去，但是这个命令不跑还不行。这时，我们可以在命令后面加上一个 \texttt{\&} 符号，表示让这个命令在后台运行，这样我们就可以继续输入其他命令了。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{shell-oneko-bg.png}
    \caption{在Linux上运行 \texttt{oneko} 命令，并让它在后台运行；然后使用 \texttt{killall} 命令来终止它的运行。}
\end{figure}
...
\begin{multicols}{2}
    ...
    \begin{itemize}
        ...
        \item  \texttt{find} 命令用于在目录中查找文件和目录。
            \begin{itemize}
                \item  \texttt{-name} ：按名称查找文件和目录。
                \item  \texttt{-type} ：按类型查找文件和目录，例如 \texttt{f} 表示文件， \texttt{d} 表示目录。
                    \texttt{-size} ：按大小查找文件，例如 \texttt{+100M} 表示查找大于100MB的文件。
                \item  \texttt{-mtime} ：按修改时间查找文件，例如 \texttt{-7} 表示查找7天内修改过的文件。
                \item  \texttt{-exec} ：对查找到的文件执行指定的命令。
            \end{itemize}
...
在终端中，重定向、管道、进程替换、Here-String、变量是四种把数据从一条命令挪到另一条命令（或文件）的核心手段。它们常被混用，但机理各不相同。

\paragraph{重定向} 重定向指的是将命令的输入或输出从默认的终端重新定向到文件或其他设备。例如，使用 \texttt{>} 可以将命令的标准输出重定向到一个文件中，使用 \texttt{<} 可以将一个文件的内容作为命令的标准输入；使用 \texttt{>>} 可以将命令的标准输出追加到一个文件中。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{shell-redirect.png}
    \caption{重定向示例：将命令的输出重定向到文件中，或将文件的内容作为命令的输入。}
\end{figure}

\paragraph{管道} 管道应当很好理解，在日常生活中通常用于描述一头进一头出的东西。在命令行中，管道指的是将一个命令的输出直接作为另一个命令的输入。例如，使用 \texttt{|} 可以将前一个命令的标准输出连接到后一个命令的标准输入。在Linux中，管道是面向字符串的，即只复制文本数据；而在PowerShell中，管道是面向对象的，前一个命令的输出会被转换成对象传递给后一个命令。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{shell-pipe.png}
    \caption{管道示例：将一个命令的输出直接作为另一个命令的输入。}
\end{figure}
...
\paragraph{进程替换} 进程替换指的是将一个命令的输出作为另一个命令的输入，但不通过匿名管道，而是通过一个特殊的文件描述符或者具名管道来实现。例如，在Linux中，使用 \texttt{<(cmd)} 可以将命令 \texttt{cmd} 的输出作为一个临时文件的输入。例如，以下两个命令是等价的：
\begin{minted}{bash}
cowsay <(fortune)
fortune | cowsay
\end{minted}

最常见的进程替换大概莫过于 \texttt{diff} 命令了。我们经常需要比较两个命令的输出是否一致，所以：
\begin{minted}{bash}
  diff <(cmd1) <(cmd2)
...
\paragraph{Here-String} Here-String指的是将一个字符串直接作为命令的输入，本质是将字符串写入一个临时文件中，然后将这个临时文件作为命令的输入。因此，Here-String的效率可能也比管道慢些。Here-String的语法是 \texttt{cmd <<< "string"}，它会将字符串 "string" 写入一个临时文件中，然后将这个临时文件作为命令 cmd 的输入。例如，以下两个命令是等价的：
\begin{minted}{bash}
cowsay <<< "$(fortune)"
fortune | cowsay
\end{minted}
...
\paragraph{变量} 变量一般指的是Shell语言编程的时候所用的、 \texttt{\$} 开头的东西，例如 \texttt{\$str} 就是一个变量。变量的值可以通过命令的输出或者用户输入来赋值。例如，以下命令将 \texttt{cmd1} 的输出赋值给变量 \texttt{str}，然后将 \texttt{str} 的值作为 \texttt{cmd2} 的输入：
\begin{minted}{bash}
str=$(cmd1)
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{shell-comparison.png}
    \caption{使用替代工具后的终端界面对比。左侧是使用默认工具ls和cat的终端界面，右侧是使用替代工具lsd和bat的终端界面。}
\end{figure}
