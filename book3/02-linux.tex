...
所有的Linux根目录基本都长这样，只多不少（例如有时候我想要挂载一个新的分区，那么大多会在根目录下创建一个新的目录来挂载这个分区）。
...
Linux系统的另一个重要特性在于繁殖安装。

我们经常说的“安装”，大多数是指从头安装，或“全新安装”，也就是把一个新的Linux系统安装到一个新的磁盘分区上，这个过程通常需要格式化磁盘、分区、安装引导程序等步骤，比较麻烦。为了解决这个问题，Linux提供了繁殖安装的方式，也就是把一个已经安装好的系统复制到另一个磁盘分区上，这样就不需要重新安装了。

对此，我们依然需要对当前的系统进行一个“快照”（但这个快照不是刚刚提到的btrfs快照，而是一个完整的系统镜像），包括引导、分区表、文件系统、需要的软件等。然后，把这个镜像转移到可移动设备上（例如U盘），最后把这个镜像写入到新的磁盘分区上，这样就完成了繁殖安装。
...
root@yourcomputer$ exit # 退出root账户。这一步极为重要！
\end{minted}
