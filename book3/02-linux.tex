...
用 sudo 的另一个好处是，因为Linux是多用户的系统，所以每一个用户用sudo做了什么都会被记录在日志里，这样就可以追踪到每一个用户的操作，增加了系统的安全性；如果直接使用root账户，那么所有操作都无法区分是哪个用户做的了。

此外，假如某服务器上有多个用户需要使用管理员权限，那么直接使用root账户就会导致安全隐患（例如无法保证已经离职的人员不再使用root账户进行操作），这样就要改动root账户的密码并通知还能够使用root账户的人员了，比较麻烦。而如果是使用sudo授权的账户，那么只需要撤销该账户的sudo权限即可。
...
而在Linux生态中，软件安装的哲学则有所不同。Linux下的软件包遵循“小而美”的原则，一个软件包通常只包含该软件自身的文件，而它所依赖的公共库（如C运行时、图形界面库等）则由系统中独立的软件包提供，由多个软件共享。这种设计使得每个软件包保持精简，也便于统一维护和升级依赖库——当某个库修复了安全漏洞，系统中所有依赖它的软件都能一次性受益。
