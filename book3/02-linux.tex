...
我们应当厘清两个概念：
\begin{description}
  \item[桌面环境] 指的是一套完整的图形界面系统，提供了一整套解决方案，包括窗口管理器、文件管理器、面板、设置中心（虽然大家一般不爱用）、终端模拟器、文本编辑器等各种工具，主打一个开箱即用的体验。常见的桌面环境有KDE、GNOME、XFCE、Cinnamon、MATE等。
  \item[窗口管理器] 指的是一套管理窗口的程序，通常不包含完整的桌面环境，搞半天还得自己拼装一些工具来使用，主打一个高度定制化的体验。常见的窗口管理器有i3、bspwm、dwm、awesome等。
\end{description}
