...
类里面除了存储数据（C++叫“属性”）以外，还可以顺便把函数（C++叫“方法”）也打包进去。
\begin{minted}{cpp}
class Point2D{
public:
...
部分属性前面，你可以发现打了 \texttt{static} 符号。这说明这个属性是静态的，属于 \textbf{类本身} ，而不是类的实例。静态属性可以通过类名直接访问，例如 \texttt{Point2D::count} 。静态属性在所有实例之间共享，因此它们的值是全局的。

符号 \texttt{->} 是一个语法糖，实际上是把 \texttt{(*p).x} 简化成了 \texttt{p->x} 的形式。
...
\end{minted}
自指的另一个作用是阻止命名冲突，例如形参和成员变量同名时，可以使用 \texttt{this} 来区分它们，实例见上文 \texttt{Book} 类的构造函数。
...
在有的时候，构造函数会导致问题。例如，如果我们想要创建一个对象，但是这个对象的构造函数需要一些复杂的参数，或者我们想要根据某些条件来决定创建哪个类的对象，那么构造函数就不太适合了。这时，我们可以使用工厂函数来创建对象。工厂函数是一个普通的函数，它返回一个类的实例。现代的C++编程一般也不建议类暴露公共的构造函数，而是通过工厂函数来创建对象，这样可以更好地控制对象的创建过程。

一般情况下，当有公共的工厂函数的时候，其构造函数一般被要求是私有的，从而强制用户只能通过工厂函数来创建对象。例如：
\begin{minted}{cpp}
class Shape {
private:
    Shape() {}  // 私有构造函数，禁止直接创建对象
public:
    static Shape* createCircle() {  // 工厂函数，创建一个圆形对象
        return new Circle();
    }
    static Shape* createSquare() {  // 工厂函数，创建一个正方形对象
        return new Square();
    }
};
\end{minted}
工厂函数大多数情况下是静态的，因为它们不需要访问类的实例属性或方法。
...
有些情况下，我们希望一个类在整个程序执行过程中有且仅有一个实例。这类操作通常见于一些全局管理类，例如日志管理器、配置管理器等。此时，我们一般使用一种特殊的设计模式，即单例模式（Singleton Pattern）。单例模式通过私有构造函数和静态成员函数来实现，确保类只能被实例化一次，并提供一个全局访问点来获取这个实例。

单例模式的具体实现分多种，比如说懒汉式（Lazy Initialization）和饿汉式（Eager Initialization）。懒汉式在第一次访问实例时才创建实例，而饿汉式在程序启动时就创建实例。
\begin{minted}{cpp}
class LazySingleton {
private:
    static LazySingleton* instance;  // 静态成员变量，存储单例实例
    LazySingleton() {}  // 私有构造函数，禁止外部创建实例
public:
    static LazySingleton* getInstance() {  // 静态成员函数，获取单例实例
        if (instance == nullptr) {  // 如果实例不存在，则创建实例
            instance = new LazySingleton();
        }
        return instance;  // 返回单例实例
    }
};
LazySingleton* LazySingleton::instance = nullptr;  // 初始化静态成员变量
\end{minted}

\begin{minted}{cpp}
class EagerSingleton {
private:
    static EagerSingleton instance;  // 静态成员变量，存储单例实例
    EagerSingleton() {}  // 私有构造函数，禁止外部创建实例
public:
    static EagerSingleton& getInstance() {  // 静态成员函数，获取单例实例
        return instance;  // 返回单例实例
    }
};
EagerSingleton EagerSingleton::instance;  // 初始化静态成员变量
\end{minted}

单例模式有时候也会引入诸如线程安全等问题。

\subsection{抽象类和Interface}

抽象类是一个不能被实例化的类，它通常包含一个或多个纯虚函数。纯虚函数是指在基类中声明但没有定义的函数，子类必须重写这些函数才能实例化对象。抽象类的主要作用是为子类提供一个公共的接口，强制子类实现某些特定的方法。

接口（Interface）是一个特殊的抽象类，它只包含纯虚函数，没有任何数据成员。接口定义了一组方法，任何实现这个接口的类都必须提供这些方法的具体实现。接口的主要作用是定义一个协议，确保不同类之间可以通过这个协议进行交互。在C++的命名逻辑中，接口大多被要求以大写字母I开头，例如 \texttt{class IShape} 。
\begin{minted}{cpp}
class IShape {  // 定义一个接口，包含一个纯虚函数area()
public:
    virtual double area() const = 0;  // 纯虚函数，子类必须实现
};
class Circle : public IShape {  // Circle类实现了IShape接口
private:
    double radius;  // 圆的半径
public:
    Circle(double r) : radius(r) {}  // 构造函数
    double area() const override {  // 实现area()方法
        return M_PI * radius * radius;  // 计算圆的面积
    }
};
\end{minted}
...
但这个东西一定要少用，因为它会引入很多问题，例如命名冲突、二义性等。更上层的问题如所谓的“菱形继承”，即一个类同时继承了两个父类，而这两个父类又继承自同一个祖先类，这时就会引入二义性问题，编译器无法确定应该调用哪个祖先类的成员函数，此时必须使用虚继承（Virtual Inheritance）来解决这个问题。

所谓虚继承，就是在继承时使用 \texttt{virtual} 关键字来声明父类为虚基类，这样就可以确保在多重继承中只有一个祖先类的实例被创建，从而避免了二义性问题。
\begin{minted}{cpp}
class A { public: void foo() { cout << "A::foo" << endl; } };
class B : virtual public A { public: void foo() { cout << "B::foo" << endl; } };
class C : virtual public A { public: void foo() { cout << "C::foo" << endl; } };
...
需要说明，模板特化虽然确实有和类的继承类似的样子，但必须要注意它们之间的区别：模板特化是针对特定类型提供不同的实现，\textbf{不会继承通用模板的成员}；而类的继承则是子类继承父类的成员，并且可以重写父类的方法。因此，在使用模板特化时，我们需要为特化类型提供完整的实现，而不能依赖于通用模板的成员。
...
C++字符串（ \texttt{std::string} ）在头文件库 \texttt{<string>} 中定义，每一个字符串是一个\textbf{对象}，而不是数组。该类提供了许多方便的方法来操作字符串，例如：
\begin{itemize}
...
\end{itemize}
当然，我们很少对流进行直接操作，更多的还是用它配合 \texttt{std::getline} 等函数来处理输入输出。 \texttt{getline} 函数能够接受三个参数：输入流、输出字符串、分隔符（默认为换行符）。它会从输入流中不停读取数据，直到遇到分隔符为止，并将读取的数据存储到输出字符串中，以便于我们进行后续处理。
\begin{minted}{cpp}
#include <iostream>
#include <sstream>
#include <string>
...
\begin{answer}
    以下是各代码段的问题和修正方法：
    \begin{enumerate}
        \item 迭代器的加减应该用 \mintinline{cpp}|std::advance(it, 3);| 。另外，迭代器不能直接转换为指针，应该使用 \mintinline{cpp}|&(*it);| 来获取指向元素的指针。
        \item 在遍历集合（其他容器也一样）时，不能添加或删除元素，因为这会使迭代器失效。因此要用STL提供的 \mintinline{cpp}|remove_if| 函数，或者先收集要删除的元素，再统一删除。
        ...
        \item std::remove并不会改变容器的大小，它只是将要删除的元素移动到容器的末尾。需要使用 \mintinline{cpp}|v.erase(std::remove(v.begin(), v.end(), 2), v.end());| 来真正删除这些元素。
