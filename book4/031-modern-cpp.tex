...
字符串视图是C++17引入的一个重要特性，定义在\verb|<string_view>|头文件中。字符串视图（\verb|std::string_view|）是一种轻量级的字符串表示方式，它并不拥有字符串数据，而是引用现有的字符串数据。这使得我们可以避免不必要的字符串拷贝，从而提高性能。也就是说：
...
通过使用\verb|std::format|，我们可以更方便地创建格式化的字符串，提高代码的可读性和维护性。这个特性和\mintinline{cpp}|std::ranges|也可以方便的结合使用，例如快速将一个数组字符串化：
\begin{minted}{cpp}
#include <format>
#include <vector>
#include <ranges>
...
相比传统的字符串拼接方式，\verb|std::format|提供了一种更现代、更安全的字符串处理方式，这个确实值得在日常编程中广泛使用。\footnote{唯一美中不足的是，很多在线测评平台还没有支持这东西！}
...
\end{minted}
实际上感觉很无趣，因为这玩意完全可以用\verb|std::optional|实现。但在较新的代码中，此法能够比旧的写法更清晰地表达意图，因此也不失为一种写法。
