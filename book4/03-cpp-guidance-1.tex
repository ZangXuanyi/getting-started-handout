...
文件流和输入输出流类似，也是一个流对象。文件流有两个：\lstinline[language=C++]|std::ifstream|用于从文件读取数据，\lstinline[language=C++]|std::ofstream|用于向文件写入数据，分别对应标准输入输出的\lstinline[language=C++]|std::cin|和\lstinline[language=C++]|std::cout|。这两个流对象提供了和标准输入输出流类似的操作方法，例如使用\texttt{>>}和\texttt{<<}运算符进行读写操作。
...
\end{minted}
在上述代码中，“读取”和“写入”两个操作被分成两个流对象而不是一个文件。这是因为C++的OOP思想，每一个对象仅负责一个职责。另，在这里也可以看出，文件流应当\textbf{自行定义}，而不是已经定义好的\lstinline[language=C++]|std::cin|和\lstinline[language=C++]|std::cout|。

\lstinline[language=C++]|ofstream|默认是覆盖模式打开文件的，如果想要以追加模式打开文件，可以使用以下方式：
\begin{minted}{cpp}
std::ofstream outfile("output.txt", std::ios::app); // 以追加模式打开输出文件
\end{minted}

当然，也可以使用一个统一的文件流对象来同时进行读写操作，这时需要使用\lstinline[language=C++]|std::fstream|类，但这时候需要指定读写模式：
\begin{minted}{cpp}
...
智能指针有三个\footnote{实际上还有第四种 \lstinline[language=C++]|std::auto\_ptr|，但是因为这个类型存在一些设计缺陷，已经在C++11中被弃用，因此这里不做介绍。}主要类型。这三个主要类型都定义在\lstinline[language=C++]|<memory>| 头文件中，分别是：
\begin{itemize}
...
void foo() {    
    ...
    \end{minted}
    上述代码中，函数 \texttt{foo} 拥有指针 \texttt{ptr} 指向的整数对象的所有权，因此它负责这个对象的创建和销毁；而在该函数结束时，也要负责销毁所有的对象，也就是释放内存。如果上述代码中没有\lstinline[language=C++]|delete ptr;| 这一行，那么就会导致内存泄漏，因为这个整数对象的内存没有被释放（但上述\lstinline[language=C++]|int a = 42;| 这一行不会导致内存泄漏，因为它是栈上的对象，会在函数结束时自动销毁）。这个实际上就是C的一个大问题：如果程序员忘记释放内存，那就会导致程序内存泄漏。
...
\begin{note}
    那两个\lstinline[language=C++]|shared_ptr|为什么会导致循环引用？这是因为智能指针对内存进行管理的方式是通过\textbf{引用计数}来实现的。每有一个\lstinline[language=C++]|unique_ptr|或者\lstinline[language=C++]|shared_ptr|指向一个对象，这个对象的引用计数就会增加1；每当一个\lstinline[language=C++]|unique_ptr|或者\lstinline[language=C++]|shared_ptr|被销毁或者重新指向其他对象时，引用计数就会减少1。当引用计数变为0时，表示没有任何智能指针指向这个对象了，这时对象的内存就会被自动释放。而\lstinline[language=C++]|weak_ptr|不会影响引用计数。
...
\end{minted}
    ...
    为了解决这个问题，我们可以使用\lstinline[language=C++]|weak_ptr|来打破循环引用。例如，我们可以让prev成为一个\lstinline[language=C++]|weak_ptr|，这样就不会增加引用计数，从而避免循环引用的问题。

    而为什么在上述\lstinline[language=C++]|List|类中，head和tail使用\lstinline[language=C++]|shared_ptr|呢？这是因为head和tail是链表的入口和出口，它们需要拥有节点的所有权，以确保链表中的节点在链表存在期间\textbf{不会}被销毁。因此，使用\lstinline[language=C++]|shared_ptr|是合适的选择。我们也可以模拟以下销毁整个链表的过程：
    \begin{enumerate}
        ...
        \item 先销毁整个\lstinline[language=C++]|List|。这时候，链表头的引用计数会减少1（变成0），而尾节点的引用计数也会减少1（变成1）。
