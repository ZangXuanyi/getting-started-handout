...
\begin{caution}
    ...
    所以说，虽然笔者个人非常厌恶和反对在任何代码中使用诸如 \mintinline{cpp}|#include <bits/c++.h>| 和 \mintinline{cpp}|using namespace std;| 之类的东西，以及大量使用全局变量、开巨大的C风格静态数组、滥用宏定义、用无意义的函数和变量名、一点注释不写、通篇魔法数字等行为，但如果你仅是为了应付考试或竞赛，那么使用这些东西也无可厚非，毕竟在考试的时候这些东西确实能帮你节省不少时间——但依然建议避免这种不好的实践。
    
    但如果要是在大作业或工程项目中这么乱写，\textbf{那就太不负责任了}，被解雇了也是活该。
\end{caution}

本章所学基本聚焦于C++14及其之前的标准，后续章节会介绍更新更好的新特性。当然，本节中为了代码的简洁起见，使用了一些诸如 \mintinline{cpp}|using namespace std;| 之类的写法，但在工程代码中应当避免使用这些写法；关于这个东西是什么、为什么不建议使用、以及如何避免使用，后续章节会详细介绍。
...
\mintinline{cpp}|std::endl| 是一个特殊的操纵符（manipulator），它的作用是输出一个换行符并刷新输出缓冲区，等价于 \mintinline{cpp}|'\n' << std::flush| 。

所谓“缓冲区”指的是输出流在输出数据时会先将数据存储在一个临时的内存区域（缓冲区）中，等到缓冲区满了或者遇到换行符时才会将数据真正输出到屏幕上。使用 \texttt{std::endl} 可以确保输出立即显示在屏幕上，而不是被缓冲起来等待下一次输出操作，这对于调试和交互式程序非常有用。

但是需要注意的是，过度使用 \texttt{std::endl} 可能会导致性能问题，因为每次使用它都会刷新输出缓冲区，这可能会增加程序的运行时间。对于需要频繁输出的情况，建议使用 \texttt{'\textbackslash n'} 来代替 \texttt{std::endl} ，以避免不必要的性能开销。

在此类输出中，如涉及运算，\textbf{必须使用括号}，否则会出现优先级错误的问题。例如：
\begin{minted}{cpp}
...
当然，使用C风格的输入输出函数 \texttt{printf} 和 \texttt{scanf} 也是可以的。例如：
\begin{minted}{cpp}
...
\texttt{<cstdio>} 是C++的C风格输入输出库，与C语言的 \texttt{<stdio.h>} 头文件内容完全一致，只不过使用了C++的命名空间（ \texttt{std} ）。不过大多数实现也允许不使用 \texttt{std::} 前缀，也允许使用 \texttt{<stdio.h>} 头文件，但我们不建议使用C的头文件。

C风格的 \texttt{std::printf} 和 \texttt{std::scanf} 速度更快，因为不需要流操作；但是它们存在一些安全隐患，例如格式化字符串攻击和缓冲区溢出等问题。我们建议在C++工程上使用更安全的 \texttt{std::cin} 和 \texttt{std::cout} 。

我们在写代码的时候{\color{red}\textbf{一定不要一句C一句C++，或者说不要一句printf一句cout（反过来也不行）}}，这样会导致缓冲区冲突，从而引发一些莫名其妙的问题。要么全用C的输入输出，要么全用C++的输入输出。

\begin{note}
  实际上， \texttt{std::cin} 和 \texttt{std::cout} 和 \texttt{printf} 和 \texttt{scanf} 区别巨大。后者是一个函数，而前者是一个“流对象”（可以理解为一个“东西”而不是一个“手段”）。它们是C++标准库中的流对象，真正负责输入输出的实际上是 \texttt{<istream>} 头文件中的 \texttt{istream::read} 和 \texttt{<ostream>} 头文件中的 \texttt{ostream::write} 方法，它们被封装进 \texttt{>>} 和 \texttt{<<} 这两个运算符（流运算符），和我们的加减乘除等运算符一样。这两个运算符必然是返回流对象的一个引用，因此可以链式调用。特别的，当输入失败的时候，会返回流对象的一个“失败”状态，因此可以通过 \texttt{std::cin.fail()} 来判断输入是否成功，也可以通过布尔上下文转换（例如 \texttt{while(std::cin>>n)} ）来判断输入是否成功。
  ...
  头文件 \texttt{<stdio.h>} 是C的头文件，而 \texttt{<cstdio>} 是这个头文件在C++中的移植版本。两者内容完全一致，只不过 \texttt{<cstdio>} 使用了C++的命名空间（ \texttt{std} ）。在现代风格的C++编程中，我们通常使用 \texttt{<iostream>} 或 \texttt{<cstdio>} 来进行输入输出操作，而不是使用 \texttt{<stdio.h>} 。
\end{note}

\paragraph{格式化输出}

有时候，我们需要对输入输出进行一些格式化操作，例如设置小数点位数、对齐方式等。除了使用 \verb|printf| 的格式化字符串外，C++提供了一些操纵符（manipulator）来实现这些功能。
\begin{itemize}
...
\end{minted}
当然这个还是太冗繁了。建议使用更现代的方式，例如C++20引入的 \texttt{std::format} 函数（需要引用头文件 \texttt{<format>} ），这个东西在之后的章节中有详细介绍。

\paragraph{格式化输入}

对于输入，则复杂得多。我们推荐同学们使用更安全的C++风格输入输出方法，也就是 \texttt{cin} 、 \texttt{cout} 、 \texttt{getline} 等。
...
\begin{tip}
    在C++中，除 \verb|std::cout| ，还有 \verb|std::cerr| 和\verb|std::clog|两个流对象用于错误输出和日志输出。它们的用法和 \verb|std::cin| 、\verb|std::cout|类似，但有一些区别：
    \begin{itemize}
        \item \verb|cout|是\textbf{行缓冲}的，也就是缓冲到换行符或缓冲区满才输出，可以重定向到文件或其他设备（例如 \verb|./program > file| ），一般用于正常输出，关联的设备是标准输出设备（stdout）。
        \item \verb|cerr|是\textbf{无缓冲}的，也就是每次输出都会立即显示，不会缓冲，常用作错误信息的输出。其关联的设备也不是标准输出设备，而是标准错误设备（stderr）。重定向方式例如 \verb|./program 2> error.log| ，其中 \verb|2>| 表示重定向标准错误输出。
...
\subsection{利用文件流进行文件读写}

在C语言的章节中，我们已经知道，可以使用 \verb|FILE| 结构体的指针来操作文件。但是这种操作有一定弊病：需要手动管理文件指针，容易出错。虽然智能指针能够部分解决手动管理文件指针的麻烦，但C++提供了更好的文件操作方式：文件流（file stream）。

文件流和输入输出流类似，也是一个流对象。文件流有两个： \mintinline{cpp}|std::ifstream| 用于从文件读取数据， \mintinline{cpp}|std::ofstream| 用于向文件写入数据，分别对应标准输入输出的 \mintinline{cpp}|std::cin| 和\mintinline{cpp}|std::cout|。这两个流对象提供了和标准输入输出流类似的操作方法，例如使用 \texttt{>>} 和\texttt{<<}运算符进行读写操作。
...
\end{minted}
在上述代码中，“读取”和“写入”两个操作被分成两个流对象而不是一个文件。这是因为C++的OOP思想，每一个对象仅负责一个职责。另，在这里也可以看出，文件流应当\textbf{自行定义}，而不是已经定义好的 \mintinline{cpp}|std::cin| 和\mintinline{cpp}|std::cout|。

\mintinline{cpp}|ofstream|默认是覆盖模式打开文件的，如果想要以追加模式打开文件，可以使用以下方式：
\begin{minted}{cpp}
std::ofstream outfile("output.txt", std::ios::app); // 以追加模式打开输出文件
\end{minted}

当然，也可以使用一个统一的文件流对象来同时进行读写操作，这时需要使用 \mintinline{cpp}|std::fstream| 类，但这时候需要指定读写模式：
\begin{minted}{cpp}
...
我们有这些读写模式可以使用：
\begin{itemize}
  \item \texttt{std::ios::in} ：以读模式打开文件。
  \item \texttt{std::ios::out} ：以写模式打开文件。
  \item \texttt{std::ios::app} ：以追加模式打开文件，写入的数据会追加到文件末尾。
  \item \texttt{std::ios::ate} ：打开文件后将文件指针移动到文件末尾，可以用于读写操作。
  \item \texttt{std::ios::trunc} ：如果文件已存在，则在打开时将其内容截断为0长度（实际上就是覆盖模式）。
  \item \texttt{std::ios::binary} ：以二进制模式打开文件，而不是文本模式。这一手段打开的文件能够避免字符转换。
\end{itemize}
这些读写模式实际上都是独热编码，也正因此，我们可以使用按位或运算符（ \texttt{|} ）来组合多个模式。

\subsection{文件的其他操作}

在C语言中，我们能够使用 \verb|remove| 等函数来对文件和目录进行操作。在C++中也有类似的功能，位于 \texttt{<filesystem>} 头文件中。这个头文件在C++17中引入，提供了一些用于文件和目录操作的类和函数。 下面是一些常用的文件操作函数：
\begin{itemize}
  \item \texttt{std::filesystem::remove(path)} ：删除指定路径的文件或目录。
  \item \texttt{std::filesystem::rename(old\_path, new\_path)} ：重命名文件或目录。
  \item \texttt{std::filesystem::copy(source, destination)} ：复制文件或目录。
  \item \texttt{std::filesystem::create\_directory(path)} ：创建一个新目录。
  \item \texttt{std::filesystem::exists(path)} ：检查指定路径是否存在。
  \item \texttt{std::filesystem::file\_size(path)} ：获取指定文件的大小。
  \item \texttt{std::filesystem::current\_path()} ：获取当前工作目录的路径。
  \item \texttt{std::filesystem::absolute(path)} ：获取指定路径的绝对路径。
  \item \texttt{std::filesystem::directory\_iterator(path)} ：迭代指定目录下的文件和子目录。
  \item \texttt{std::filesystem::space(path)} ：获取指定路径所在文件系统的空间信息。
  \item \texttt{std::filesystem::last\_write\_time(path)} ：获取指定文件的最后修改时间。
  \item \texttt{std::filesystem::permissions(path, perms)} ：设置指定文件或目录的权限。
  \item \texttt{std::filesystem::remove\_all(path)} ：递归删除指定路径的文件或目录及其内容。
\end{itemize}

需要注意的是， \verb|path| 是一个类型，可以是字符串类型（如 \verb|std::string| ）或者 \verb|std::filesystem::path| 类型。使用这些函数时，需要包含头文件 \verb|<filesystem>| ，并且在编译时需要链接文件系统库（例如使用 \verb|-lstdc++fs| 选项）。
...
在现代 \texttt{C++} 中， \texttt{const} 常变量不依靠运行时初始化来确定其值（例如 \mintinline{cpp}|const int b = 1;| ），其表现就和 \texttt{constexpr} 常量一样了。因此，在大多数时候，我们也可以把 \texttt{const} 常变量当作 \texttt{constexpr} 常量来使用。但如希望严谨表达意图，仍建议使用 \texttt{constexpr} 来声明常量。
...
\begin{enumerate}
  \item 分配一块更大的内存空间，通常是当前容量的两倍。
  \item 将原有元素从旧内存空间复制到新内存空间。
  \item 释放旧的内存空间。
  \item 更新动态数组的内部指针和容量信息。
\end{enumerate}
...
堆内存的分配和释放需要程序员手动管理，使用 \texttt{new} 关键字分配内存，使用 \texttt{delete} 关键字释放内存。堆内存的空间较大，可以达到几GB，但分配和释放的速度较慢，且容易导致内存泄漏和碎片化问题。
...
\end{minted}
上述强枚举的枚举名被限定在作用域内，因此只能通过类似 \texttt{Color::RED} 来访问。强枚举的底层类型可以显式指定，例如上面的 \texttt{std::uint8\_t} 。如果不指定，默认是 \texttt{int} 。调用这种枚举只能使用上述的正统调用：
\begin{minted}{cpp}
...
智能指针有三个\footnote{实际上还有第四种 \mintinline{cpp}|std::auto\_ptr|，但是因为这个类型存在一些设计缺陷，已经在C++11中被弃用，因此这里不做介绍。}主要类型。这三个主要类型都定义在\mintinline{cpp}|<memory>| 头文件中，分别是：
\begin{itemize}
  \item \texttt{std::unique\_ptr} ：表示独占所有权的强引用智能指针，一个对象只能有一个 \texttt{unique\_ptr} 指向它。当这个 \texttt{unique\_ptr} 被销毁时，所指向的对象也会被自动释放。
  \item \texttt{std::shared\_ptr} ：表示共享所有权的强引用智能指针，一个对象可以有多个 \texttt{shared\_ptr} 指向它。对象会在最后一个指向它的 \texttt{shared\_ptr} 被销毁时自动释放。
  \item \texttt{std::weak\_ptr} ：表示弱引用的智能指针，不拥有对象的所有权。它通常与 \texttt{shared\_ptr} 一起使用，用于解决循环引用的问题。
\end{itemize}

\begin{note}
    上述文字中，提到了一个新的名词：\textbf{所有权}。这是编程中的一个非常重要的概念。
...
void foo() {    
    ...
    \end{minted}
    上述代码中，函数 \texttt{foo} 拥有指针 \texttt{ptr} 指向的整数对象的所有权，因此它负责这个对象的创建和销毁；而在该函数结束时，也要负责销毁所有的对象，也就是释放内存。如果上述代码中没有 \mintinline{cpp}|delete ptr;| 这一行，那么就会导致内存泄漏，因为这个整数对象的内存没有被释放（但上述\mintinline{cpp}| int a = 42;| 这一行不会导致内存泄漏，因为它是栈上的对象，会在函数结束时自动销毁）。这个实际上就是C的一个大问题：如果程序员忘记释放内存，那就会导致程序内存泄漏。
...
\end{minted}
除非直接定义，否则不建议直接使用智能指针的构造函数来创建智能指针对象，而是推荐使用 \texttt{make\_unique} 和 \texttt{make\_shared} 这两个工厂函数来创建智能指针对象。例如：
\begin{minted}{cpp}
...
\end{minted}
\verb|make_unique| 和 \verb|make_shared| 是创建智能指针的推荐方式。我们要创建什么类型、指向什么对象，就在尖括号和圆括号中指定即可。这样可以避免许多的潜在错误，例如内存泄漏、悬空指针等。
...
\begin{note}
    那两个 \mintinline{cpp}|shared_ptr| 为什么会导致循环引用？这是因为智能指针对内存进行管理的方式是通过\textbf{引用计数}来实现的。每有一个 \mintinline{cpp}|unique_ptr| 或者 \mintinline{cpp}|shared_ptr| 指向一个对象，这个对象的引用计数就会增加1；每当一个 \mintinline{cpp}|unique_ptr| 或者 \mintinline{cpp}|shared_ptr| 被销毁或者重新指向其他对象时，引用计数就会减少1。当引用计数变为0时，表示没有任何智能指针指向这个对象了，这时对象的内存就会被自动释放。而 \mintinline{cpp}|weak_ptr| 不会影响引用计数。
...
\end{minted}
    ...
    为了解决这个问题，我们可以使用 \mintinline{cpp}|weak_ptr| 来打破循环引用。例如，我们可以让prev成为一个 \mintinline{cpp}|weak_ptr| ，这样就不会增加引用计数，从而避免循环引用的问题。

    而为什么在上述 \mintinline{cpp}|List| 类中，head和tail使用 \mintinline{cpp}|shared_ptr| 呢？这是因为head和tail是链表的入口和出口，它们需要拥有节点的所有权，以确保链表中的节点在链表存在期间\textbf{不会}被销毁。因此，使用 \mintinline{cpp}|shared_ptr| 是合适的选择。我们也可以模拟以下销毁整个链表的过程：
    \begin{enumerate}
        \item 在链表的生命周期内：链表头的引用计数是1，链表尾节点的引用计数是2（“链表尾”\footnote{“链表尾”和“链表尾节点”不是一个东西。以一个长度为10的链表为例，链表尾节点是第10个节点，而链表尾是一个指向第10个节点的指针。}指向尾节点，而前一个节点也指向尾节点）。
        \item 先销毁整个 \mintinline{cpp}|List| 。这时候，链表头的引用计数会减少1（变成0），而尾节点的引用计数也会减少1（变成1）。
        \item 由于链表头的引用计数变成0，因此头节点会被自动释放。头节点的释放会导致它的next指针指向的下一个节点的引用计数减少1，也就是第二个节点的引用计数变成0。
        \item 于是产生链式反应，整个链表的节点都会被依次释放，直到尾节点。
    \end{enumerate}
\end{note}

这是写了一个双向链表节点的例子。注意其中的 \texttt{next} 是强引用，而 \texttt{prev} 是弱引用。这样可以避免循环引用的问题，从而防止内存泄漏。这个指针在多线程编程中也很有用，可以安全地访问共享资源。当然，我们一般不去主动构造一个没有配合的弱引用，弱引用大多都是依附于共享强引用而存在的。

而数组中也不再建议使用指针来访问了，改为统一使用STL容器，例如 \texttt{std::vector} 、 \texttt{std::array} 的\textbf{迭代器}。于是，裸指针在C++中基本光荣退休。至于迭代器是啥，我们会在后续章节中详细介绍。
...
命名空间符号在编译时是在符号前加上命名空间前缀来实现的。例如， \texttt{std::cout} 在编译时会被转换为类似 \texttt{\_ZSt4cout} 这样的符号名（具体名称可能因编译器而异）。这样做的好处是可以避免不同命名空间中的符号冲突。
...
\end{minted}
上述代码中， \texttt{\_Z3addii} 和 \texttt{\_Z3adddd} 分别是两个 \texttt{add} 函数的修饰名称，包含了参数类型的信息，从而使得编译器能够区分它们。但是我们发现一个问题：函数的返回值是不参与名称修饰的，这意味着如果两个函数只有返回值类型不同，而参数列表相同，那么它们仍然会冲突，编译器会报错。

但这就会导致一个问题：如果我们需要在C++中调用C语言的函数怎么办？因为C语言不支持名称修饰，而C++支持名称修饰，这就会导致链接错误。为了解决这个问题，C++提供了 \texttt{extern "C"} 关键字，可以告诉编译器按照C语言的方式来处理函数名称，从而避免名称修饰。例如：
\begin{minted}{cpp}
extern "C" {
    void c_function(int a);
}
\end{minted}
上述代码中， \texttt{c\_function} 函数会按照C语言的方式来处理名称，不会进行名称修饰，从而可以正确地链接C语言的函数。当然，这也会有所牺牲，在该块中的函数和变量不能使用C++的特性，例如函数重载、命名空间等。
...
\textbf{必须注意， \texttt{auto} 不是一个类型，而是一个占位符，编译器会根据变量的初始值来推断出一个具体的类型。} 因此，使用 \texttt{auto} 时必须确保变量有一个明确的初始值，否则编译器无法推断出类型，会导致编译错误。我们也绝对不能把 \texttt{auto} 当成一个万能的类型来使用，例如：
\begin{minted}{cpp}
...
\end{minted}
不能把它当Python那样的动态类型来使用。如确实需要动态类型，可以使用 \texttt{std::variant} 或 \texttt{std::any} 来实现，或使用泛型编程（模板）来实现类型的灵活性。

但是依然需要注意：\texttt{std::variant} 和 \texttt{std::any} 以及模板虽然可以实现类型的灵活性，但它们也不是动态类型，他们仅仅是一个盒子，里面可以装不同类型的东西，但这个盒子本身的类型是确定的。具体使用见C++高级特性那一章。

因此，我们需要根据实际需求来选择合适的工具，而不是滥用 \texttt{auto} 或者其他类型灵活性的工具。

\begin{caution}
  不要滥用 \texttt{auto} ，因为这会使得代码的可读性降低，尤其是当变量的类型不明显时。我们只推荐在\textbf{变量的类型是确定的}、\textbf{类型的名称非常冗长}、\textbf{你知道这个变量的类型是什么或者大概是什么}的情况下使用类型推断，例如 \mintinline{cpp}|auto it = std::max_element(v.begin(), v.end());| 中， \texttt{it} 的类型是 \texttt{std::vector<int>::iterator} 是确定的，这个类型名称很长，你也知道它是个迭代器类型，这时候使用 \texttt{auto} 是非常合适的。但是如果你真写了 \mintinline{cpp}|auto x = 5;| 这种代码，那就属于滥用了，是不提倡的。
...
\begin{enumerate}
  ...
  \item \textbf{其他转换}：包括数组到指针、函数到指针、空指针常量、枚举到整型\footnote{ \texttt{C++11} 起的强枚举不能隐式转换}、类类型的转换\footnote{指的是诸如代码： \mintinline{cpp}|struct A{ A(int);}; void foo(A); foo(42);| 中， \texttt{foo(42)} 把整数隐式转换为类}等。
\end{enumerate}
...
如果源文件数量过多，那么就不应该使用诸如gcc等工具手动地编译。此时，应该使用 \texttt{Make} 、 \texttt{CMake} 、 \texttt{XMake} 、 \texttt{Conan} 等构建工具来管理编译过程。关于后三个构建工具的使用，将在下一章中介绍。
...
\begin{answer}
    ...
    \begin{enumerate}
        \item \mintinline{cpp}|auto| 用作声明变量时，必须有初始值，否则编译器无法推断类型。改正方法： \mintinline{cpp}|auto x = 42;| 。
        \item 范围for循环中， \mintinline{cpp}|auto v| 是按值传递的，修改 \mintinline{cpp}|v| 不会影响原数组。改正方法：使用引用传递， \mintinline{cpp}|for (auto& v : a)| 。
        \item 函数默认参数值只能在函数声明中指定，不能在函数定义中指定。改正方法：在 \texttt{main.cpp} 中添加函数声明时指定默认参数值：
\begin{minted}{cpp}
int add(int a, int b = 0);
\end{minted}
        \item \texttt{constexpr} 函数的参数必须是编译时常量， \mintinline{cpp}|n| 是运行时输入的变量，不能作为 \texttt{constexpr} 变量的初始化值。
        \item C++的标准输出流和C的标准输出使用不同的缓冲区，可能导致输出顺序不确定。改正方法：使用同一种输出方式，或者在 \texttt{printf} 之后调用 \texttt{std::cout.flush()} 。
        \item \texttt{nullptr} 可以转换为任何指针类型，因此调用 \texttt{f(nullptr)} 时，编译器无法确定调用哪个重载版本。改正方法：显式指定调用的版本，例如 \mintinline{cpp}|f(static_cast<int*>(nullptr))| 。
    \end{enumerate}
\end{answer}
