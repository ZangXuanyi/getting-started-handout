...
使用CLI也可以编译运行这个程序，假设你的文件名是 \texttt{hello.c} ，你可以在终端中输入以下命令：
\begin{minted}{bash}
gcc hello.c -o hello
./hello
\end{minted}
...
\begin{caution}
  ...
  笔者个人从工程的眼光上看来，非常不建议弄出 \texttt{a = b++} 这类的代码，尽管这类代码在竞赛中会让很多OIer感到Tricky，但是在工程中会让人无比恼火。如果想先用b的值再加1，可以写成 \texttt{a = b; b++;} ；如果想先加1再用b的值，可以写成 \texttt{b++; a = b;} 。上述写法一般只有非常约定俗成的场合才会使用，例如 \texttt{while(T--)} 或者 \texttt{stk[++top]=x} ——不过即使是我，也更习惯于写成 \mintinline{cpp}|for(;T>0;T--)| 和 \mintinline{cpp}| stack<int> stk; stk.push(x);|。
\end{caution}
...
一个条件表达式，最简单的情况肯定是“真”或“假”。C语言规定：true等价于1，false等价于0；但非0的数值还是什么别的非空的东西全部视作true。因此， \texttt{if (1)} 和 \texttt{if (-42)} 甚至 \texttt{if (3.14)} 和 \texttt{if ("hello")}都肯定会执行，而 \texttt{if (0)} 则肯定不会执行。
...
\begin{tip}
  在C/C++中，不能使用类似 \texttt{1 <= x <= 2} 这样的连续记号来表示区间。正确的写法是 \texttt{(x >= 1) \&\& (x <= 2)} ，即把每个比较都单独写出来，然后用逻辑与运算符连接起来。

  一般情况下，如果比较变量和字面值，一般把字面值放在比较符的右边，例如 \texttt{x >= 1} 。其他则无所谓，一般需要遵循团队的编码规范。
\end{tip}

在C/C++中，与或非运算符是有一定的运算顺序的。一般情况下，逻辑非运算符的优先级最高，其次是逻辑与运算符，最后是逻辑或运算符。不过笔者非常不建议同学们背诵这个顺序；实际在工程上不仅不建议大量嵌套使用这些运算符，而且遇事不决可以加括号——括号可比记运算顺序靠谱得多了！
...
为了理解这些内容，我们首先要知道：\textbf{我们写的东西，本质上最终都会被编译器转换成机器码，而机器码是需要内存空间来存储数据的}。
...
\begin{itemize}
  \item 声明：告诉编译器指定对象的名称和类型，但不为其分配内存空间。
  \item 定义：为\textbf{声明过的}对象分配内存空间，并提供实现或提供一个初始值。
\end{itemize}
...
int a; // 声明并定义变量a
\end{minted}
上述代码既声明了变量 \texttt{a} ，也为其分配了内存空间。有的同学可能会疑惑：这哪里分配内存空间了？实际上，编译器在看到上述代码时，会为变量 \texttt{a} 分配4个字节的内存空间（假设 \texttt{int} 类型占4个字节），并将其初始化为一个值（虽然这个数值可能是垃圾值）。

只有在少数情况下，对于特定变量的声明才能分开。这种情况大多见于多文件编程中，例如：
\begin{minted}{c}
extern int b; // 仅声明变量b，不定义
\end{minted}
上述代码只是声明了变量 \texttt{b} ，并没有为其分配内存空间。变量 \texttt{b} 的定义需要在其他地方进行，例如：
\begin{minted}{c}
int b; // 定义变量b，分配内存空间
\end{minted}
上述代码为变量 \texttt{b} 分配了内存空间，并将其初始化为一个值。
...
printf("%d\n", a);
\end{minted}
上述代码中，变量 \texttt{a} 被定义了两次，导致编译失败。
...
printf("%f\n", a);
\end{minted}
这个也是错误的，因为变量 \texttt{a} 的类型不一致，出现了“冲突声明”（conflicting declaration）。

\paragraph{变量的作用域与生命周期}

变量的作用域是指变量在程序中可见和可访问的范围。在C语言中，变量的作用域向下延伸至其定义所在的代码块（block）。代码块是由一对大括号包围的代码区域。一般的，人为地将变量分为“全局”（不在任何块内声明）和“局部”（在某个块内声明）两种。一般情况下，全局变量的作用域是整个文件，甚至可以跨文件访问（通过 \texttt{extern} 关键字）。局部变量作用域仅限于该代码块内部。
...
\begin{itemize}
  \item \texttt{auto} ：默认的存储类，表示局部变量，生命周期为代码块执行期间。换句话说，我们常见的 \verb|int a;| 实际上等价于 \verb|auto int a;|。
  \item \texttt{register} ：建议将变量存储在CPU寄存器中，以提高访问速度，但编译器可以忽略此建议。常用于寄存器优化，但除非写内核或嵌入式，否则一般没有人写这个，而是让编译器自己决定。
  \item \texttt{static} ：表示静态变量，生命周期为整个程序运行期间，但作用域为定义所在的代码块。常用于需要在多次函数调用之间保持值的变量。
  \item \texttt{global} ：表示全局变量，生命周期为整个程序运行期间，作用域为整个文件，甚至可以跨文件访问（通过 \texttt{extern} 关键字）。实际上和全局定义的 \verb|int a;| 是等价的。
  \item \texttt{extern} ：表示外部变量，声明在其他文件中定义的变量，用于跨文件访问。
  \item \texttt{const} ：表示常变量，值不可修改。
\end{itemize}
...
一个更常见的写法是使用 \texttt{typedef} 关键字为结构体定义一个别名，这样在声明结构体变量时就不需要再写 \texttt{struct} 了。例如：
\begin{minted}{c}
...
想象内存是一条无限长的一维停车场。停车场被划分为许多连续编号的地块，这被叫做“地址”（address）。现在开来了一辆车，这辆车需要占用若干个连续的地块，这就相当于我们声明了一个变量。不同类型的变量需要占用不同数量的地块，例如 \verb|int| 需要4个地块， \verb|char| 需要1个地块。抽象理解一下，变量在内存上其实就是一块连续的字节区域，例如 \verb|int| 占4字节，那么它在内存上就是4个连续的字节； \verb|char| 占1字节，那么它在内存上就是1个字节。不同类型的变量在内存上占用的字节数是不同的。按理说，将所有变量密铺在内存上即可，而这确实是朴素的处理方式。
...
于是，编译器厂家等灵光一闪：为什么不能人为规定，让变量在内存上的起始地址必须是其大小的整数倍呢？这样一来，变量就不会跨越多个字，CPU在访问这些变量时就可以一次性完成，从而提高访问速度。这个过程就叫做“内存对齐”（memory alignment）。故而， \verb|int| 的起始地址必须是4的倍数， \verb|char| 的起始地址可以是任意地址， \verb|double| 的起始地址必须是8的倍数，依此类推。

回到停车场比喻上：假设我们来了一辆拖车，上面载着两个小车，一个是 \verb|int| ，一个是 \verb|char| 。停车场管理员（CPU）登记拖车的同时，还要把小车一起登记（改变其在拖车上的位置）；又规定， \verb|int| 必须停在4的倍数号的地块上，而 \verb|char| 可以停在任意地块上。那么，如果这个拖车（结构体）从地块0开始停放，那么 \verb|int| 就必须停在地块4上，而 \verb|char| 只能停在地块0上；中间的地块1、2、3就只能空着，作为填充（padding）字节。这样一来，拖车就占用了地块0到7，共8个地块，而不是5个地块。这个道理和结构体的内存对齐一致，因此上述结构体的大小是8字节。
...
联合体和结构体实际上是类似的，所以“定义联合体结构”和“声明联合体变量”自然也是分开的；同理，也可以用 \texttt{typedef} 为联合体定义别名，或使用不具名联合体。

联合体的内存模型极为简单，其大小等于其最大成员的大小，这是所有成员共享同一块内存的结果。例如，上述联合体 \texttt{Data} 的大小为4字节，因为 \texttt{int} 和 \texttt{float} 都占用4字节。而下列联合体
\begin{minted}{c}
union Example {
    char a;      // 1字节
    double b;    // 8字节
...
\end{minted}
的大小为8字节，因为 \texttt{double} 是最大的成员，占用8字节。
...
我们发现，在定义上述 \texttt{add} 函数时，使用了两个参数 \texttt{a} 和 \texttt{b} 。这两个参数在函数内部是可以使用的，但是在函数外部是无法访问的。这是因为函数参数的作用域仅限于函数内部。换句话说，函数参数在函数外部是不可见的，无法被访问或修改。
...
现在我们 \texttt{int a = 42;} 。于是，编译器给a分配了4个连续的地块（假设 \texttt{int} 类型占4字节），并把这4个地块的编号（地址）记为0x1000、0x1001、0x1002、0x1003。于是，我们可以说变量a的“地址”是0x1000，告诉我们变量a停放在内存的哪个位置；而它的值是42，告诉我们变量a里面存放的东西是什么。
...
\begin{note}
    ...
    C语言程序员大多习惯 \verb|int *p = &a;| ，认为这样更符合语言的习惯；而C++程序员大多习惯 \verb|int* p = &a;| ，认为这样更能体现指针类型的本质（但“指针类型”这个概念实际上是C++引入的，C语言并没有这个概念）。实际的代码应符合团队的代码风格规范。为了便于理解，我们在本书中统一使用 \verb|int* p = &a;| 这种写法。
\end{note}
可以看到，上述 \texttt{\&a} 就是“取地址”，也就是获取变量a的地址。而 \texttt{int* p} 则是声明了一个指针变量p，它的类型是“指向int类型变量的指针”，也就是说，p只能存储int类型变量的地址。于是，我们就可以说，p存储了变量a的地址0x1000。
...
\begin{minted}{c}
    *p = 100; //
    ...
    printf("%d\n", a); // 输出变量a的值
\end{minted}
我们惊奇的发现，虽然我们看似修改的是p，但p并没有改变，但a变了！这是为什么呢？这是因为 \texttt{*p}被称作“解引用”（dereference），表示“通过指针p访问它所指向的变量”，也就是“通过地址0x1000访问地块里的东西”。因此 \texttt{*p = 100;} 的意思就是“把p指向的地块里的东西改成100”，也就是把变量a的值改成100。
...
不要反复 \verb|*(&(*(&p)))| 多重嵌套乱玩。在嵌套次数过多的时候，编译器不能正确解析。

有一种特殊的指针被称为“空指针”（null pointer），可以理解为“该指针没有指向任何地块”，常用作为指针的初始值或者表示指针不指向任何有效变量。在C中，可以使用宏 \texttt{NULL} 来表示空指针。例如：
\begin{minted}{c}
...
\end{minted}
符号 \texttt{->} 是一个语法糖，实际上是把 \texttt{(*p).x} 简化成了 \texttt{p->x} 的形式。当然，\texttt{(*p).x} 也是合法的写法，但要打出来还是较为麻烦的。
...
\subsection{初级泛型编程：void指针}

在C语言中， \texttt{void*} 指针是一种特殊的指针类型，它可以指向任何类型的数据。由于 \texttt{void*} 指针没有具体的类型信息，因此在使用时需要进行类型转换（type casting）才能访问其指向的数据。但这给了程序员极大的自由度，很多C程序员都是 \texttt{void*} 大神。

首先必须说明：\texttt{void*} 指针不能进行解引用操作，因为编译器无法确定其指向的数据类型和大小；也不能对其进行加减运算，因为编译器无法确定每次移动的字节数。故而，实际上只能把 \texttt{void*} 指针作为一种通用的指针类型来传递数据，而不能直接操作数据。
...
\end{minted}
在上述代码中，函数 \texttt{printValue} 接受一个 \texttt{void*} 指针和一个类型标识符作为参数。根据类型标识符的值，函数将 \texttt{void*} 指针转换为相应类型的指针，并通过解引用操作访问其指向的数据。这是很初级的“泛型”编程：通过 \texttt{void*} 指针，我们可以编写能够处理不同数据类型的通用函数，从而提高代码的复用性和灵活性。

但这种操作也是极为令人头疼的，因为你必须自己保证传递的类型标识符和实际数据类型一致，否则就会导致未定义行为；而有些代码库中，甚至调用某个函数还会返回 \texttt{void*} 指针，不深入理解的情况下甚至不知道这是个什么东西，令人并不愉快。所以一定要谨慎使用 \texttt{void*} 指针。
...
在C中，文件操作主要通过标准库中的 \texttt{FILE} 结构体和相关函数来实现。 \texttt{FILE} 结构体表示一个文件流，它包含了文件的状态信息和缓冲区等数据。我们可以使用指向 \texttt{FILE} 结构体的指针来操作文件。为了使用文件操作功能，我们需要包含头文件 \texttt{stdio.h} 。

例如，我们需要使用以下手段来进行文件操作：
\begin{itemize}
    \item 打开文件：使用 \texttt{fopen} 函数打开一个文件，返回一个指向 \texttt{FILE} 结构体的指针。
    \item 读取文件：使用 \texttt{fread} 或 \texttt{fgets} 等函数从文件中读取数据。
    \item 写入文件：使用 \texttt{fwrite} 或 \texttt{fputs} 等函数向文件中写入数据。
    \item 关闭文件：使用 \texttt{fclose} 函数关闭文件，释放资源。
\end{itemize}
...
\mintinline{c}|fopen| 函数接受两个参数：文件名（实际上是文件相对可执行文件的路径）和模式。模式可以是r（只读）、w（只写，文件不存在则创建，存在则清空）、a（追加写入，文件不存在则创建）等。函数返回一个指向 \texttt{FILE} 结构体的指针，如果打开失败则返回 \texttt{NULL} 。

\subsection{文件操作}

除了文件的读写外，我们有时候还希望删除、重命名文件等操作。C标准库提供了一些函数来实现这些功能，例如：
\begin{itemize}
  \item \texttt{remove(filename)} ：删除指定的文件。
  \item \texttt{rename(old\_filename, new\_filename)} ：重命名文件。
  \item \texttt{fseek(file\_ptr, offset, whence)} ：设置文件指针的位置。
  \item \texttt{ftell(file\_ptr)} ：获取文件指针的当前位置。
  \item \texttt{rewind(file\_ptr)} ：将文件指针重新定位到文件的开头。
  \item \texttt{feof(file\_ptr)} ：检查是否到达文件末尾。
  \item \texttt{ferror(file\_ptr)} ：检查文件操作是否出错。
\end{itemize}
例如，我们想删除一个文件，可以使用 \texttt{remove} 函数：
\begin{minted}{c}
...
\end{minted}
\verb|perror| 函数用于打印最近一次系统调用失败的错误信息。
...
头文件（header files）通常以 \texttt{.h} 作为文件扩展名，包含函数声明、宏定义、结构体定义等内容。源文件（source files）通常以 \texttt{.c} 作为文件扩展名，包含函数的具体实现和程序的入口点（ \texttt{main} 函数）。通过将代码拆分成头文件和源文件，我们可以实现代码的模块化和重用。我们可以在多个源文件中包含同一个头文件，从而共享函数声明和数据结构定义。例如：
\begin{minted}{c}
// math_utils.h
#ifndef MATH_UTILS_H
#define MATH_UTILS_H
...
\end{minted}
在上述代码中，我们将数学运算函数的声明放在头文件 \texttt{math\_utils.h} 中，将函数的实现放在源文件 \texttt{math\_utils.c} 中，然后在主程序 \texttt{main.c} 中包含头文件并调用这些函数。

头文件中的 \texttt{\#ifndef} 、 \texttt{\#define} 和 \texttt{\#endif} 是一种常见的防止重复包含的技术，称为“包含保护”（include guard），也叫“编译守卫”。它确保头文件只被包含一次，避免了重复定义的问题。

\subsection{include原理}

我们可能会疑惑：为什么在上述编译指令中，只需要指定源文件，而不需要指定头文件呢？编译守卫又是怎么起作用的呢？这是因为头文件实际上并不是独立的编译单元，而是通过预处理指令 \texttt{\#include} 被包含到源文件中的。为了理解这个，我们先简要理解一下编译的过程。

编译大致分为以下几个阶段：
\begin{itemize}
    \item 预处理（Preprocessing）：处理预处理指令，如 \texttt{\#include} 、 \texttt{\#define} 等，除去注释，生成纯净的源代码。这一步也叫“预编译”。
    \item 编译（Compilation）：将预处理后的源代码转换为汇编代码。
    \item 汇编（Assembly）：将汇编代码转换为机器码，生成目标文件（通常以 \texttt{.o} 或 \texttt{.obj} 作为扩展名）。
    \item 链接（Linking）：将多个目标文件和库文件链接成一个可执行文件。
\end{itemize}

\texttt{\#include} 是一种\textbf{预编译指令}，其意思可以理解为“把指定的头文件的内容直接插入到当前位置”。因此，当我们编译源文件时，编译器会先处理所有的 \texttt{\#include} 指令，将头文件的内容插入到源文件中，然后再进行编译。换句话说，在预编译这一步结束后，上述的 \texttt{math\_utils.c} （实际上不会叫这个名字了）会变成这样：
\begin{minted}{c}
int add(int a, int b);
int subtract(int a, int b);
int add(int a, int b) {
...
所以说，如果没有这个头文件，那么编译器在编译 \texttt{main.c} 时就会报错，提示找不到 \texttt{add} 和 \texttt{subtract} 的声明。

\subsection{链接原理}

在编译过程中，编译器会把多个源文件分别处理成多个目标文件。而把多个“目标文件”真正合并成一个可执行文件的过程，叫做“链接”（linking）。链接器会把各个目标文件中的符号（函数和变量）进行解析和合并，从而生成最终的可执行文件。

具体的合并过程大致是这样的。如果我们直接编译上述 \texttt{main.c}，并查看其汇编，会发现：
\begin{minted}{gas}
    call    add         0x00401000 <add> # 这是胡编的地址，仅作示例
    call    subtract    0x00401010 <subtract> # 同上
\end{minted}
可以看到，编译器并没有把 \texttt{add} 和 \texttt{subtract} 函数的具体实现插入到 \texttt{main} 函数中，而是生成了对这两个函数的调用指令，并且使用了占位符地址。这是因为编译器确实看到了这两个函数的声明，但并不知道这两个函数的具体实现在哪里，故而知道它们是\textbf{外部函数}。故而，编译器会在该源文件的目标文件中，生成对这两个外部函数的“标记”（也就是\textbf{符号}），表示“这里要使用add函数，地址待定”。

而真正定义这两个东西的是 \texttt{math\_utils.c} ，它会被编译成另一个目标文件，其中包含了这两个函数的具体实现和地址信息，即\textbf{符号定义}。

链接器的核心职责就是把这堆东西合并成一个，其最重要的任务就是“符号解析”：
\begin{itemize}
    \item 扫描所有的目标文件，收集每一个文件提供的符号定义和需要的符号引用。
    \item 对每一个符号引用，查找对应的符号定义，并将引用地址替换为定义地址。
    \item 处理未定义符号，如果有符号引用找不到对应的定义，链接器会报错。
\end{itemize}
以上述代码为例，链接器在处理 \texttt{main.o} 时，发现它引用了 \texttt{add} 和 \texttt{subtract} 这两个符号；然后它扫描 \texttt{math\_utils.o} ，发现它定义了这两个符号，并且知道它们的具体地址；最后，链接器将 \texttt{main.o} 中对这两个符号的引用地址替换为它们在 \texttt{math\_utils.o} 中的实际地址，从而完成链接过程。
...
该库主要负责输入输出操作。除了 \mintinline{c}|scanf| 和\mintinline{c}|printf|，还包括文件操作等功能。
\begin{itemize}
...
\begin{itemize}
  \item \texttt{malloc(size)} ：分配指定大小的内存块。
  \item \texttt{calloc(num, size)} ：分配指定数量的内存块，并将其初始化为零。
  \item \texttt{free(ptr)} ：释放之前分配的内存块。
...
\begin{enumerate}
    \item n未初始化就使用。应先读入n，再定义数组。另，VLA不是C标准的一部分，建议使用动态内存分配 \mintinline{c}|malloc| 。
    \item 数组越界。应改为 \mintinline{c}|i < 5| 。
    \item 指针未初始化就使用。
    \item 数组长度不足以存储输入的字符串，是忘记 \verb|\0| 导致的。应改为 \mintinline{c}|char str[6];| 。
    \item 误用赋值运算符。应改为 \mintinline{c}|if (x == 0)| 。
    \item 返回局部变量地址，导致悬空指针。应改为静态变量或动态分配内存。
    \item 数组作为函数参数时退化为指针，sizeof结果是指针大小。应传入数组大小作为额外参数。
    \item 未定义行为，因为 \mintinline{c}|a++| 的副作用未定义顺序。应改为两行分别处理。
\end{enumerate}
改正代码略，读者可自行完成。
\end{answer}
