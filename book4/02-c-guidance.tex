...
\begin{caution}
  ...
  笔者个人从工程的眼光上看来，非常不建议弄出 \texttt{a = b++} 这类的代码，尽管这类代码在竞赛中会让很多OIer感到Tricky，但是在工程中会让人无比恼火。如果想先用b的值再加1，可以写成 \texttt{a = b; b++;} ；如果想先加1再用b的值，可以写成 \texttt{b++; a = b;} 。上述写法一般只有非常约定俗成的场合才会使用，例如 \texttt{while(T--)} 或者 \texttt{stk[++top]=x} ——不过即使是我，也更习惯于写成\mintinline{cpp}|for(;T>0;T--)| 和 （C++ 语法）\mintinline{cpp}|stack<int> stk; stk.push(x);|。
\end{caution}
...
\begin{tip}
  在 C 中，不能使用类似 \texttt{1 <= x <= 2} 这样的连续记号来表示区间。正确的写法是 \texttt{(1 <= x) \&\& (x <= 2)} ，即把每个比较都单独写出来，然后用逻辑与运算符连接起来。
\end{tip}
...
\begin{itemize}
  ...
  \item \texttt{register} ：建议将变量存储在CPU寄存器中，以提高访问速度，但编译器可以忽略此建议。常用于寄存器优化，但除非写内核或嵌入式，否则一般没有人写这个，而是让编译器自己决定。被 \texttt{register} 关键字修饰的变量不能被取地址。
  \item \texttt{static} ：表示静态变量，生命周期为整个程序运行期间，但作用域为定义所在的代码块。常用于需要在多次函数调用之间保持值的变量。
  \item \texttt{global} ：（global 不是关键字）以 \verb|int a;| 形式定义的全局变量，生命周期为整个程序运行期间，作用域为整个 C 程序，通过 \texttt{extern} 关键字来跨文件访问变量。
...
具体的合并过程大致是这样的。如果我们直接编译上述 \texttt{main.c}，并查看其反汇编，会发现：
\begin{minted}{gas}
    call	0x3d <main+0x3d> # call	add@PLT
    call	0x4c <main+0x4c> # call subtract@PLT
\end{minted}
