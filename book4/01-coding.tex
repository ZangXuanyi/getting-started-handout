...
\begin{caution}
  如果你是信科的同学，或将来有志于从事相关研究工作（如计算机视觉、物理仿真、自然语言处理、数据分析等），请务必看完本章内容，因为这些工作对编程环境有更高的要求，而信科的课程则天天和代码打交道，所以学会配置环境更是非常必要的。
...
\begin{note}
  Visual Studio 把一个工作目录视为一个“项目”，当用户使用编译功能时会把整个项目编译一遍。而在同一个项目中，不能同时存在多个同名的变量和函数，因此即使我们在VS中新建了不同的C++文件且他们在逻辑上并无任何关联，但是VS依然会把它们视为同一个项目的一部分，从而导致命名冲突的问题。对于初学者来说，这无疑是一个巨大的坑，往往表现为“我这个 \verb|main| 函数怎么出现了重定义的错误？”。

  另外，VS出于安全性考量，禁用了C系的一部分函数，例如 \verb|printf| 。当我们试图对这些函数进行调用时，VS会报错并提示我们使用更安全的版本，例如 \verb|printf_s| 。这无疑给初学者带来了不必要的困扰。当然这个困扰并非不能解决，只需要在文件开头定义宏 \verb|#define _CRT_SECURE_NO_WARNINGS| 即可，但这无疑增加了学习的难度。另外， \verb|printf_s| 等并不是标准函数，这会导致代码的可移植性变差。

  综上所述，我非常建议新手远离VS。每一年都会有大量的同学因为使用VS而陷入困境，浪费了大量的时间和精力。
\end{note}

为了解决臃肿的问题，我们一般使用轻量的文本编辑器来编写代码，并用自己额外安装的编译器或解释器来编译运行代码。这样做的好处是可以根据自己的需求选择合适的工具，避免了臃肿和不必要的功能，同时也提高了灵活性和可定制性。常见的文本编辑器包括Vim、Visual Studio Code、notepad++、Kate等。
...
\begin{tip}
  退出VIM的命令是 \texttt{:q} ，如果你在编辑器中输入了内容并且想要保存，可以使用 \texttt{:wq} 命令；如果你不想保存，可以使用 \texttt{:q!} 命令强制退出。纵然如此，我个人还是建议同学们学着使用一下vim这个经典TUI编辑器，这是因为在将来的开发中，我们或多或少都会面对一些情况：不得不从远端登录某机器，且这个机器甚至还不能安装一些诸如nano等更现代的编辑器！
\end{tip}

笔者个人非常推荐新手使用Visual Studio Code（简称VS Code或者Code），它是一款轻量级的编辑器，具有良好的扩展性和社区支持，可以满足不同用户的需求。VS Code支持多种编程语言，并且有丰富的插件生态系统，可以根据需要安装各种插件来增强功能。它同时也为调试和版本控制功能添加了GUI，非常适合初学者和中小型项目开发者使用。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.3\textwidth]{103/fake-vs.jpg}
  \caption{假货}
\end{figure}
...
Kate也是我个人非常喜欢的一个编辑器，原出自KDE社区，后来支持了多个平台。其界面与VS Code非常类似，但更为轻量，尤其适合超轻量开发。其插件生态虽然较VS Code逊色，但对于一些特定的语言（例如LaTeX）有着非常好的支持。如果你觉得VS Code还是太过臃肿了，不妨试试Kate。不过使用Kate的前提是，你大概必须自己先把编译工具链和调试工具链都配好，并熟悉如何在命令行中使用它们。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{103/regex.png}
...
\end{minted}
注意应当安装\texttt{mingw-w64-ucrt-x86\_64-git}包，它是原生Windows 版本；而名为\texttt{git}的包是Cygwin版本，性能较差且可能与VS Code等工具兼容性不佳。Git for Windows本身其实也是基于MSYS2构建的精简环境，因此一般在MSYS2中管理时没有必要再单独安装Git for Windows。
...
\paragraph[UI配置]{UI配置\protect\footnote{本节作者张天齐。}}
...
\begin{itemize}
  ...
  \item 编译器参数：留空即可。如需要，可以添加一个 \texttt{-O2} 或者 \texttt{-O3} 来开启编译优化，或者添加一个 \texttt{-g} 来开启调试信息。但是， \texttt{-g} 会严重拖慢编译速度，因此建议只在调试时开启。另，如果想使用较新的C++特性，也可以加上 \verb|--std=c++23| 之类的选项。
...
另外，如果希望使用Code的插件来帮助你编译并运行C/C++代码，则需要打开一个工作文件夹。否则，插件无法为你提供编译和运行的功能。当然， \mintinline{bash}|g++ main.cpp -o main.exe && ./main.exe| 也并非不可。
...
命令行能提供更高的灵活性，可以随意增删改编译选项，例如添加 \texttt{-Wall} 来开启警告等。我个人非常推荐同学们学会使用命令行来编译和运行代码，这样可以更好地理解编译过程，并且能够更好地控制编译选项。
...
\section{Python、虚拟环境及其配置}\label{sec:conda}

\subsection{简单安装Python}
...
\paragraph{conda} conda是数据科学领域开山鼻祖级别的包管理和虚拟环境管理工具。它不仅可以管理Python包，还可以管理其他语言的包，例如R、Ruby等，因此也是迄今为止用的最广泛的跨语言包管理工具。但conda有两个缺点：一个是笨重，一个是慢。
...
del .\miniconda.exe
\end{minted}
Linux用户见\href{https://www.anaconda.com/docs/getting-started/miniconda/install}{官方网站}即可，其实大差不差。
...
\end{minted}
上述命令的含义是：创建一个名为 \texttt{myenv} 的虚拟环境，并安装Python 3.12版本。你可以根据需要更改环境名称和Python版本。在快速开发的背景下，可以把东西一股脑的安装到base环境中，但不推荐把这种手段用到正式开发中。

\paragraph{mamba} mamba是conda的一个替代品，旨在解决conda的慢的问题。mamba使用C++编写，具有更快的包解析和安装速度。mamba完全兼容conda的命令和配置文件，因此用户可以无缝切换到mamba，而无需修改现有的环境和脚本。

如果计算机上已经安装了conda，则可以通过以下命令来安装mamba：
\begin{minted}{bash}
conda install mamba -n base -c conda-forge
\end{minted}
安装完成后，你可以使用 \texttt{mamba} 命令来代替 \texttt{conda} 命令，例如：
\begin{minted}{bash}
...
\end{minted}
然后按照提示操作即可。安装完成后，你可以使用 \texttt{micromamba} 命令来创建和管理虚拟环境，使用方法与conda类似。
...
\begin{note}
在手敲以下代码的过程中，请注意输入法保持在\textbf{英文}输入状态，编译器会认为中文全角标点为标识符，而缺少分隔符，于是报错。
\end{note}
