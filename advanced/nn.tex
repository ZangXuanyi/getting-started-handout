...
我们已经训练好了一个模型。现在的问题是：我们怎么知道这个模型好不好呢？

我们已经知道，衡量模型好坏，最权威的标准就是它在测试集上的表现——损失函数的值越小越好。但损失函数的具体含义随着任务的不同而不同，我们迫切地需要一个更具体可感的指标来衡量模型的性能。回归问题我们在高中的时候就知道可以用残差平方和（RSS）来衡量模型究竟拟合得多好（实际上依然是损失函数）。而对于离散的分类问题，一个朴素的指标就是准确率，即“分对的占总数的比例”。

这个指标在大多数情况下是相当好的，但考虑以下问题：假设现在训练一个模型预测某个数是不是素数。该模型总是输出“不是素数”。于是，随着N的增大，该模型的准确率趋近于1，但这个模型显然是没有任何实际意义的。这个问题在机器学习中被称为\textbf{类别不平衡问题}，即某些类别的样本数量远多于其他类别，导致模型倾向于预测多数类，从而获得高准确率但实际性能很差。

为了解决这个问题，我们引入了更多指标来衡量模型的性能：
\begin{description}
    \item[精确率] 在所有被模型预测为正类的样本中，实际为正类的比例，即“模型声称是素数的数字中，确实是素数的比例”。
    \item[召回率] 在所有实际为正类的样本中，被模型正确预测为正类的比例，即“所有素数中，被模型正确识别为素数的比例”。
    \item[F1分数] 精确率和召回率的调和平均数，综合考虑了模型的精确性和召回能力。 
\end{description}
所以上述模型的精确率不能计算（除零错误），但召回率为0，说明模型的实际性能非常差。

这些指标也能在日常生活中有所体现。例如为了防控艾滋病，很多人提出“国家应当对艾滋病进行全面普查”，以便及时发现感染者并进行隔离治疗。那么我们构思以下模型：存在一个艾滋病检测手段准确率为99.9\%，即如果一个人感染了HIV，则有99.9\%的概率报告“阳性”，剩下0.1\%即“未检出”；如果一个人没有感染HIV，则有99.9\%的概率报告“阴性”，剩下0.1\%即“假阳性”。

假设现在有100万人，其中1000人感染了HIV。根据上述检测手段，我们可以计算出：
\begin{itemize}
    \item 真阳性（True Positives, TP）：1000人中有999人被正确检测为阳性。
    \item 假阴性（False Negatives, FN）：1000人中有1人被错误检测为阴性。
    \item 真阴性（True Negatives, TN）：999000人中有998001人被正确检测为阴性。
    \item 假阳性（False Positives, FP）：999000人中有999人被错误检测为阳性。
\end{itemize}
...
现实很好反映了这一点：根据FDA等权威机构的数据，HIV快速检测的灵敏度（召回率）超过99\%（还不如我上面为了计算方便编的数据呢），特异性（真阴性率）接近99.9\%；但以美国2019-2024年对2190万次检测的真实世界研究为例，特异性达到99.86\%，精确率68.4\%\footnote{数据来源：\url{https://journals.lww.com/jaids/fulltext/2026/05010/real_world_positive_predictive_value_and_false.9.aspx\#ContentAccessOptions}}，所以这就是为什么HIV检测不推荐对普通人群进行普查，而是采用风险分层筛查策略。

更何况，普查还要考虑大量的其他问题：光看一元一份的胶体金试剂，对全国进行普查仅仅需要14亿元的开销（对于国家而言，14亿元并不是一个大数目）；但是，光是组织筛查这一项工作，就需要用到大量的人力和物力；已经知道自己感染了HIV的人也不太可能乖乖地去接受筛查（甚至就算是现在对HIV感染者进行保密，老老实实去治疗的人也不算多，反而自暴自弃、报复社会的人也不少），所以普查的实际执行难度也是非常大的；更重要的是，误诊为阳性的人（即假阳性的140万人并不是一个小数目）也大概率会产生心理压力和社会歧视，引发这样那样的社会问题。综上所述，这个想法虽然很美好，但在现实中是很难行得通的。

但这不能说明我们的检测手段是不好的。实际上在医院中，该手段必然是辅助检测艾滋病的重要手段，因为召回率非常高，且医生几乎只会对高风险人群进行检测，这些高风险人群的感染率不可能低，例如性工作者的感染率大致在10\%左右，那么10000人中就有1000人感染了HIV，那么在被报告为阳性的$999+9 = 1008$人中，真正感染者的比例就达到了99.1\%，精确率就非常高了。这个例子也很好地说明了：\textbf{不同的应用场景需要不同的指标来衡量模型的性能}，我们不能简单地说一个模型好坏，而是要看它在特定场景下的表现。
...
一个非常常见的笑话：在不限制规律复杂度的情况下，任何找规律题目的答案都可以是42，因为42是宇宙的终极答案（划去）因为拉格朗日插值多项式可以完美拟合任何有限数据集，或者说总能找到一个k次多项式$f(x)$，使得$f(x+1)=42$。但是这样的高阶多项式会剧烈震荡，在训练数据点之间的区域表现很差，且系数往往也非常大，导致数值不稳定。
...
批量归一化在卷积里极为常用，又稳又快还能折叠权重，是现代神经网络的标配。
