...
那么，\mintinline{cpp}|std::vector<T>|是个什么东西呢？实际上，这东西也是顺序表，但它的实现方式更复杂一些。它会预留一部分内存空间，当需要插入元素时，如果当前容量不够，它会分配一块更大的内存空间，然后把原来的数据拷贝过去，再插入新的元素。这样做的好处是，\mintinline{cpp}|std::vector<T>|在大多数情况下在末尾插入和删除元素的时间复杂度是$O(1)$，只有在需要扩容的时候才是$O(n)$；而在中间插入和删除元素的时间复杂度仍然是$O(n)$。
...
在C++标准库中，\mintinline{cpp}|std::list<T>|就是一个双向链表的实现。我们不要自己试图实现一个链表，直接使用标准库中的实现即可。
...
栈是一种只能在一端进行插入和删除操作的数据结构。我们可以使用\mintinline{cpp}|std::stack<T>|来实现栈：
\begin{minted}{cpp}
#include <stack>
...
队列和栈类似，但它允许在一端插入元素，在另一端删除元素。我们可以使用\mintinline{cpp}|std::queue<T>|来实现队列：
\begin{minted}{cpp}
#include <queue>
...
双端队列是一种特殊的队列，允许在两端进行插入和删除操作。我们可以使用\mintinline{cpp}|std::deque<T>|来实现双端队列：
\begin{minted}{cpp}
#include <deque>
...
\end{itemize}
而删除的过程更是复杂，不再赘述。C++标准库中的\mintinline{cpp}|std::map<K, V>|和\mintinline{cpp}|std::set<T>|就是基于红黑树实现的，我们可以直接使用它们来存储有序的键值对或集合。
...
在C++中，哈希表可以通过\mintinline{cpp}|std::unordered_map<K, V>|和\mintinline{cpp}|std::unordered_set<T>|来实现：
\begin{minted}{cpp}
...
\paragraph{链地址法}
链地址法是将每个数组位置存储为一个链表（或其他数据结构），当发生冲突时，将新元素添加到该位置的链表中。查找时，需要遍历链表以找到目标元素。这样的实现方式在C++的\mintinline{cpp}|std::unordered_map|和\mintinline{cpp}|std::unordered_set|中被广泛使用。
...
在C++中的堆是通过\mintinline{cpp}|std::priority_queue<T>|来实现的：
\begin{minted}{cpp}
#include <queue>
...
maxHeap.pop(); // 删除堆顶元素
\end{minted}
优先队列实际上也是一个“队列”，但内部并不是简单的先进先出，而是根据元素的优先级来决定出队顺序。最大堆的优先级是值越大越高，最小堆则是值越小越高。在C++中优先队列也是一个容器适配器，底层通常使用\mintinline{cpp}|std::vector<T>|来存储元素，并通过堆操作来维护堆性质；也可以自定义底层容器，但必须满足随机访问迭代器的要求。
