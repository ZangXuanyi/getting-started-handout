...
如果你已经按照前文建议安装了MSYS2，那么可以直接在MSYS2中安装Git，无需单独下载。详见前文\ref{sec:coding}节。
...
\begin{enumerate}
  \item 在自己的仓库中创建一个新的分支。
  \item 在新的分支上进行开发，完成某个功能或修复某个问题。
  \item 把该分支推到远程仓库中。
  \item 创建一个PR，请求将自己的分支合并到主分支。
  \item 自己审自己（或者其他开发者一起审，提出建议）。
  \item 合并，删除远端临时分支。
  \item 本地回到主分支，拉取远程仓库的代码。
  \item 删除本地临时分支。
\end{enumerate}
...
\verb|github-cli| 是 GitHub 提供的一个命令行工具，可以帮助我们在命令行中进行 GitHub 相关的操作，例如创建 PR、查看 Issues 等。使用 github-cli 可以让我们在命令行中更方便地管理 GitHub 仓库和项目。不同的Linux发行版大多提供了该工具的包，也可以使用winget和Homebrew等包管理器来安装。
...
在此之后，我们便可以通过 \verb|github-cli| 来进行一些 GitHub 相关的操作了，例如创建 PR：
\begin{minted}{bash}
...
\end{minted}
上述\verb|!| 表示这是一个 shell 命令，而不是 Git 的内置命令；\verb|$1| 表示第一个参数，即 PR 的标题。这样，我们就可以使用以下命令来创建 PR 和合并 PR 了：
\begin{minted}{bash}
# 前面照搬照抄
...
该工具还提供了很多其他的功能，例如查看 PR 列表、查看 Issues 列表、查看仓库信息等。我们可以使用 \verb|gh help| 命令来查看所有可用的命令和选项，或者直接访问 GitHub CLI 的官方文档来了解更多信息。
...
\paragraph{线性的提交} 这是最常见的情况。我们在本地完成一些开发，提交到本地仓库，然后推送到远程仓库中。这个过程非常简单，使用以下命令：
\begin{minted}{bash}
git add .
git commit -m "commit message"
git push origin main
...
\paragraph{提出PR} 现在我们是协助别人开发。我们Fork了一个仓库，在本地做出了一些修改，现在希望把这些修改合并到原仓库中。此时，我们有两个方法。
...
\paragraph{合并PR} 当我们是PR的维护者时，我们需要审查PR，并决定是否合并。如果PR没有问题，我们可以直接在GitHub上点击“Merge”按钮来合并PR。我们非常建议使用变基策略或压缩提交的方式来合并PR，这样可以保持提交历史的整洁。如果我们使用github-cli来合并PR，可以使用以下命令：
\begin{minted}{bash}
gh pr merge --squash --delete-branch
\end{minted}
这样会将 PR 合并到目标分支，并且使用 squash 的方式合并（即把多个提交压缩成一个提交），最后删除临时分支。有时候可能无需删除该临时分支（例如这个分支是别人的）。

如果想要采用变基的方式来合并PR，则将上述命令中的 \verb|--squash| 替换为 \verb|--rebase|。变基会比合并更好地保持历史的线性，但不直观。

在实际的仓库中，务必保证主要分支提交历史的线性。具体采用变基还是压缩，还是不压缩，取决于维护者的偏好和团队的习惯。无论如何，保持提交历史的整洁和可读性是非常重要的。

\paragraph{维护仓库} 当我们本人是仓库的维护者，且该仓库有很多人使用时，我们依然建议使用PR的方式开发，以与其余人保持一致。
...
git commit --amend
\end{minted}
这会将当前的修改添加到上一次提交中，并且允许我们修改提交信息。如果提交信息没有问题，我们可以添加一个 \verb|--no-edit| 。

如果这个错误已经被推送到远程仓库中，并且我们需要修改这个错误，那么我们在持有强制推送权限的情况下，可以使用以下命令来推送提交：
\begin{minted}{bash}
git push --force-with-lease origin main
\end{minted}
这里的 \verb|--force-with-lease| 是一个安全的强制推送选项，它会在推送前检查远程仓库是否有新的提交，如果有，则拒绝推送，以避免覆盖他人的工作。不建议单用 \verb|--force|，因为它会直接覆盖远程仓库的代码，可能会导致其他工作丢失。
...
\paragraph{处理合并冲突（本地）} 当我们试图提交PR的时候，可能会遇到合并冲突的情况。这时，除了使用GitHub等给的编辑器之外，我们还可以在本地来解决合并冲突，随后重新提交PR。具体的大致流程是，我们要先把待合并入的分支拉到本地，然后在本地利用更趁手的编辑器来解决冲突，最后再把修改后的代码推送到远程仓库中。以下是具体的命令：
\begin{minted}{bash}
git checkout main
git pull origin main 
git checkout feature
...
\paragraph{紧急修改bug} 当我们发现生产环境中的代码出现了紧急问题时，我们需要立即修复这个问题。如果在使用Git Flow的公司中，我们不能容忍常规流程的效率，此时可以特事特办，先签出一个 \verb|hotfix| 分支修复这个问题，然后将其直接合并进 \verb|main| 分支，最后再将 \verb|main| 分支合并进 \verb|develop| 分支以同步代码。此时的修改可以包括临时的、不稳定的处理方式，等到问题彻底解决之后，再在 \verb|develop| 分支上进行更稳定的修复，并将其合并进 \verb|main| 分支。
...
\paragraph{分支交错} 有时候粗心把一些提交交到了错误的分支上。此时，我们并不需要摘樱桃。我们可以直接在本地使用 \verb|git rebase| 来修改提交的基底，从而把这些提交移到正确的分支上。以下是具体的命令：
\begin{minted}{bash}
git checkout feature
...
\end{minted}
这里是利用了创建分支时分支会指向当前提交的特性来实现的。如果把 \verb|HEAD~3| 改成 \verb|origin/main|，则可以把这个分支重置到远程仓库的 main 分支上，尤其多见于忘记了分支保护规则，将一些提交直接提交到该分支上时的补救措施。

\paragraph{暂存} 有时候某个功能写到一半，需要紧急去做一些其他的工作。但我们不希望提交这个写到一半的功能。此时，我们可以这么做：
\begin{minted}{bash}
git stash
# do something else...
git stash pop
\end{minted}
\verb|git stash| 会把当前的修改保存到一个临时的栈里面，并且恢复到上一次提交的状态。这样我们就可以去做其他的工作了。当我们完成其他的工作之后，可以使用 \verb|git stash pop| 来把之前保存的修改恢复回来。

\paragraph{撤销一个已经被Merge的PR} 当我们发现一个已经被合并的PR出现了问题时，我们需要撤销这个PR。此时，我们可以使用以下命令来撤销这个PR：
\begin{minted}{bash}
git checkout main
git pull origin main
...
git push origin main
\end{minted}
这里的 \verb|hash| 是指那个合并提交的哈希值。该命令是较为友好的撤销方式，它会创建一个新的提交来撤销之前的合并提交，而不会直接修改提交历史。这样可以保证代码的安全性和稳定性。不建议直接 \verb|reset| 然后强制推送，除非团队很小，全员知情。

\paragraph{意外提交敏感信息} 假设我们在本地有一个 \verb|.env| 文件存储了密钥或密码等敏感信息，却忘记将这个文件添加到 \verb|.gitignore| 文件中，结果不小心把该文件提交到仓库中去了。此时，我们需要立即撤销（或修改）该提交，并将此文件从git缓存中清除，并阻止git跟踪该文件。
...
\end{minted}
如果更早的提交中也有，此时必须用交互式 \verb|rebase| 来修理提交历史，或使用其他手段，如 \verb|filter-branch| 或 \verb|BFG Repo-Cleaner| 来彻底删除该文件的历史记录。无论如何，即使这样做，也可能会清理不干净的情况，建议修改密码或轮转密钥，来确保安全。